...
object ReaderDbManager {
    private lateinit var roomDb: ReaderDatabase
    private val dbName = "netnovelreader.db"

    fun initDatabase(context: Context) {
        roomDb = Room.databaseBuilder(context, ReaderDatabase::class.java, dbName).addCallback(dbCallBack).build()
    }

    fun shelfDao() = roomDb.shelfDao()

    fun sitePreferenceDao() = roomDb.sitePreferenceDao()

    fun runInTransaction(block: () -> Unit) = roomDb.runInTransaction { block() }
    ...
    fun dropTable(tableName: String?) {
        ...
        synchronized(ReaderDbManager::class) {
            roomDb.openHelper.writableDatabase.execSQL("drop table if exists $tableName;")
        }
    }
    ...
    fun getChapterNameAndUrl(tableName: String, isDownloaded: Int): LinkedHashMap<String, String> {
        ...
        val cursor = roomDb.openHelper.writableDatabase.query(
    ...
    fun setReaded(tableName: String, id: Int): ArrayList<String> {
        ...
        cursor.close()
        roomDb.openHelper.writableDatabase.execSQL("update $tableName set ${ReaderDatabase.ISDOWNLOADED}='2' where ${ReaderDatabase.ID}<=$id;")
        return arrayList
    }
    ...
    fun delChapterAfterSrc(tableName: String, chapterName: String): ArrayList<String> {
        ...
        cursor.close()
        roomDb.openHelper.writableDatabase.execSQL("delete from $tableName where ${ReaderDatabase.ID}>=$id")
        return arrayList
    }
    ...
    fun getChapterCount(tableName: String): Int {
        ...
        val cursor =
            roomDb.openHelper.writableDatabase.query("select count(*) from $tableName;", null)
        if (cursor.moveToFirst()) {
            c = cursor.getInt(0)
        }
        cursor.close()
