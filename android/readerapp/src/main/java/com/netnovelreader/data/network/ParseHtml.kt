...
class ParseHtml {
    ...
    fun getCatalog(url: String, selector: String): LinkedHashMap<String, String> {
        ...
        val list: Elements = Jsoup.connect(url).headers(getHeaders(url)).timeout(TIMEOUT).get()
    ...
    private fun getChapterWithOutSelector(url: String): String {
        val elements = Jsoup.connect(url).headers(getHeaders(url)).timeout(TIMEOUT).get().allElements
