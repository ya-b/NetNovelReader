...
object ReaderDbManager {
    ...
    private fun getRoomDB(): ReaderDatabase {
    ...
    fun shelfDao() = getRoomDB().shelfDao()

    fun sitePreferenceDao() = getRoomDB().sitePreferenceDao()

    fun runInTransaction(block: () -> Unit) = getRoomDB().runInTransaction { block() }
    ...
    fun getChapterCount(tableName: String): Int {
        ...
        val cursor =
                getRoomDB().openHelper.writableDatabase.query("select count(*) from $tableName;", null)
        if (cursor.moveToFirst()) {
            c = cursor.getInt(0)
        }
        cursor.close()
