...
class ShelfActivity : AppCompatActivity(), IShelfContract.IShelfView {
    ...
    override fun initView(){
        ...
        if(checkPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE)){
            shelfRecycler.adapter = BindingAdapter(mViewModel?.getModel()?.bookList,
                    R.layout.item_shelf_recycler_view, ShelfViewModel.ShelfClickEvent())
            updateShelf(null)
        }else{
    ...
    override fun updateShelf(adapter: BindingAdapter<ShelfModel.BookInfoBean>?) {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        if(requestCode == 1){
            if(grantResults[0] == PackageManager.PERMISSION_GRANTED){
                updateShelf(shelfRecycler.adapter as BindingAdapter<ShelfModel.BookInfoBean>)
            }else{
