...
class LoginDao : ILoginDao {
    ...
    override fun addUser(user: UserBean) {
        val sql =
            "replace into ${DBHelper.TABLE_USER} (${DBHelper.USERNAME}, ${DBHelper.PASSWD}, ${DBHelper.EMAIL}, ${DBHelper.ROLE}) values " +
    ...
    override fun delUser(username: String) {
        QueryRunner().update(
            DBHelper.getConn(), "delete from ${DBHelper.TABLE_USER} where ${DBHelper.USERNAME} = " +
                    "'$username';"
        )
        DBHelper.closeConn()
    }
    ...
    override fun getUser(username: String): UserBean? {
        val result = QueryRunner().query(
            DBHelper.getConn(), "select * from ${DBHelper.TABLE_USER} where ${DBHelper.USERNAME} " +
